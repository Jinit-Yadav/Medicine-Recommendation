...
try:
    ...
    print("Error loading model. Please ensure svc.pkl exists.")
    svc = None
...
def helper(dis):
    ...
    pre = [col for col in pre.values[0]] if len(pre.values) > 0 else []

    med = medications[medications['Disease'] == dis]['Medication']
    med = [m for m in med.values] if len(med.values) > 0 else []

    die = diets[diets['Disease'] == dis]['Diet']
    die = [d for d in die.values] if len(die.values) > 0 else []

    wrkout = workout[workout['disease'] == dis]['workout']
    wrkout = [w for w in wrkout.values] if len(wrkout.values) > 0 else []
...
def predict():
    try:
        ...
        result = {
            ...
            'medications': med,
            'diets': die,
            'workouts': wrkout.tolist() if hasattr(wrkout, 'tolist') else wrkout
        }
